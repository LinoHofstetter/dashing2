...
int dashing2_main(int argc, char **argv, DistanceCallback callback, dashing2::SketchingResult sketch1, dashing2::SketchingResult sketch2, bool cmp_objects) {
    ...
    if(argc > 1) {
        ...
        if(std::strcmp(argv[1], "cmp") == 0 || std::strcmp(argv[1], "dist") == 0) {
            ...
            if (verbosity >= Verbosity::DEBUG){ //added for debugging
                std::cout << "Sketch 1 name: " << sketch1.names_[0] 
                    << ", cardinality: " << sketch1.cardinalities_[0] 
                    << ", signatures[1023]: " << sketch1.signatures_[1023] 
                    << ", signatures size: " << sketch1.signatures_.size() 
                    << ", begin: " << *(sketch1.signatures_.begin()) 
                    << ", end: " << *(sketch1.signatures_.end() - 1) 
                    << std::endl;
                std::cout << "Sketch 2 name: " << sketch2.names_[0] 
                    << ", cardinality: " << sketch2.cardinalities_[0] 
                    << ", signatures[1023]: " << sketch2.signatures_[1023] 
                    << ", signatures size: " << sketch2.signatures_.size() 
                    << ", begin: " << *(sketch2.signatures_.begin()) 
                    << ", end: " << *(sketch2.signatures_.end() - 1) 
                    << std::endl;
